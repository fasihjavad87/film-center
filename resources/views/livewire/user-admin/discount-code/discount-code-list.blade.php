        ...
        <div class="card-body">
            ...
            <div class="flex flex-col-reverse md:flex-row md:justify-between items-start">
                ...
                <div class="flex gap-x-1 w-full md:w-max justify-between">
                    @if(auth()->user()->isAdmin('create-discount-code'))
                        <a href="{{ route('panelAdmin.discountCodes.create') }}"
            ...
            <div class="flex justify-center items-center">
                @if($discountCodes->isEmpty())
                    <span class="text-red-500 px-3.5 py-1.5 border-[1.4px] border-red-500 rounded-md">هیچ تخفیفی وجود ندارد!</span>
                @else
                        ...
                        <div class="parent-table w-max md:w-full overflow-auto md:overflow-hidden"
                            ...
                            <table class="box-table">
                                ...
                                @foreach($discountCodes as $discountCode)
                                    <tr class="table-item">
                                        <td><span class="font-normal">{{ $discountCode->code }}</span></td>
                                        <td>
                                            <span class="font-normal">
                                                <span class="text-green-500">{{ $discountCode->percent }}%</span>
                                             </span>
                                        </td>
                                        <td><span class="font-normal">{{ $discountCode->max_usage ?? 0 }} کاربر</span>
                                        </td>
                                        <td>
                                            @php
                                                $expiresData = $discountCode->expiresInDays();
                                            @endphp
                                            <span class="{{ $discountCode->expiresInClasses() }}">
                                                {{ $expiresData['label'] }}
                                            </span>
                                        </td>
                                        <td>
                                            <span class="{{ $discountCode->statusClasses() }}">
                                                    {{ $discountCode->statusLabel() }}
                                            </span>
                                        </td>
                                        <td class="flex justify-center items-end gap-x-2.5">
                                            @if(auth()->user()->isAdmin('edit-discount-code'))
                                                <a href="{{ route('panelAdmin.discountCodes.edite', $discountCode->id ) }}"
                                            ...
                                            @endif
                                            @if(auth()->user()->isAdmin('delete-discount-code'))
                                                <a href="#"
                                                   wire:click.prevent="openDeleteModal({{ $discountCode->id }} , '{{ $discountCode->code }}')"
            ...
            <div class="flex justify-center mt-10">
                <div class="flex space-x-2 rtl:space-x-reverse">
                    {{ $discountCodes->links('vendor.pagination.custom-pagination-panel-tailwind') }}
                </div>
            </div>
            ...
            <div wire:loading.remove wire:target="openDeleteModal" class="w-full">
                ...
                <div class="flex flex-col">
                    ...
                    <div class="flex items-center gap-x-1.5">
                        ...
                        <!-- عنوان (h3) -->
                        <h3 class="text-lg leading-6 font-bold text-red-600 dark:text-red-400">تأیید حذف کد تخفیف <span
                                class="text-indigo-600 dark:text-indigo-400">{{ $discountCodeNameToDelete }}</span></h3>
                    </div>

                    {{-- ردیف ۲: متن اصلی (با تورفتگی برای تراز) --}}
                    <div class="mt-3 mr-4">
                        <p class="text-sm text-gray-700 dark:text-gray-300">
                            آیا مطمئن هستید که می‌خواهید کد تخفیف «<span
                                class="text-red-600 dark:text-red-400 font-medium">{{ $discountCodeNameToDelete }}</span>»
                            را برای همیشه حذف کنید؟ این عمل غیرقابل بازگشت است.
                        </p>
                    </div>
                </div>
