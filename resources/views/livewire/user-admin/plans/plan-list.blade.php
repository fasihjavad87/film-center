        ...
        <div class="card-body">
            ...
            <div class="flex flex-col-reverse md:flex-row md:justify-between items-start">
                ...
                <div class="flex gap-x-1 w-full md:w-max justify-between">
                    @if(auth()->user()->isAdmin('create-plan'))
                        <a href="{{ route('panelAdmin.plans.create') }}"
            ...
            <div class="flex justify-center items-center">
                @if($plans->isEmpty())
                    <span class="text-red-500 px-3.5 py-1.5 border-[1.4px] border-red-500 rounded-md">هیچ اشتراکی وجود ندارد!</span>
                @else
                        ...
                        <div class="parent-table w-max md:w-full overflow-auto md:overflow-hidden"
                            ...
                            <table class="box-table">
                                ...
                                @foreach($plans as $plan)
                                    <tr class="table-item">
                                        <td><span class="font-normal">{{ $plan->name }}</span></td>
                                        <td><span class="font-normal">{{ $plan->duration_days }} روز</span></td>
                                        <td><span
                                                class="font-normal">{{ number_format($plan->price) }} <span>تومان</span></span>
                                        </td>
                                        <td>
                                            <span class="font-normal">
                                                    {{-- از متد جدید استفاده می‌کنیم که بالاترین درصد رو برمی‌گردونه --}}
                                                @if ($plan->effectiveDiscountPercent() > 0)
                                                    <span class="text-green-500">{{ $plan->effectiveDiscountPercent() }}%</span>
                                                @else
                                                    <span class="text-red-500">بدون تخفیف</span>
                                                @endif
                                            </span>
                                        </td>
                                        <td>
                                            @if ($plan->hasDiscount() )
                                                <span class="text-blue-600 font-bold">
                                                        {{ number_format($plan->finalPrice()) }} تومان
                                                </span>
                                            @else
                                        ...
                                        <td>
                                <span class="{{ $plan->statusClasses() }}">
                                    {{ $plan->statusLabel() }}
                                </span>
                                        </td>
                                        <td class="flex justify-center items-end gap-x-2.5">
                                            @if(auth()->user()->isAdmin('edit-plan'))
                                                <a href="{{ route('panelAdmin.plans.edite', $plan->id ) }}"
                                            ...
                                            @endif
                                            @if(auth()->user()->isAdmin('delete-plan'))
                                                <a href="#"
                                                   wire:click.prevent="openDeleteModal({{ $plan->id }} , '{{ $plan->name }}')"
            ...
            <div class="flex justify-center mt-10">
                <div class="flex space-x-2 rtl:space-x-reverse">
                    {{ $plans->links('vendor.pagination.custom-pagination-panel-tailwind') }}
                </div>
            </div>
            ...
            <div wire:loading.remove wire:target="openDeleteModal" class="w-full">
                ...
                <div class="flex flex-col">
                    ...
                    <div class="flex items-center gap-x-1.5">
                        ...
                        <!-- عنوان (h3) -->
                        <h3 class="text-lg leading-6 font-bold text-red-600 dark:text-red-400">تأیید حذف پلن <span
                                class="text-indigo-600 dark:text-indigo-400">{{ $planNameToDelete }}</span></h3>
                    </div>

                    {{-- ردیف ۲: متن اصلی (با تورفتگی برای تراز) --}}
                    <div class="mt-3 mr-4">
                        <p class="text-sm text-gray-700 dark:text-gray-300">
                            آیا مطمئن هستید که می‌خواهید پلن «<span
                                class="text-red-600 dark:text-red-400 font-medium">{{ $planNameToDelete }}</span>» را
