        ...
        <div class="card-body">
            ...
            <div class="flex flex-col-reverse md:flex-row md:justify-between items-start">
                ...
                <div class="flex gap-x-1 w-full md:w-max justify-between">
                    @if(auth()->user()->isAdmin('create-category'))
                        <a href="{{ route('panelAdmin.categories.create') }}"
                ...
                <div class="parent-table w-max md:w-full overflow-auto md:overflow-hidden" wire:key="user-list-table">
                    <table class="box-table">
                        ...
                        @foreach($categories as $category)
                            <tr class="table-item">
                                <td><span class="font-normal">{{ $category->name }}</span></td>
                                <td><span class="font-normal">{{ $category->e_name }}</span></td>
                                <td><span class="font-normal">{{ $category->slug }}</span></td>
                                <td class="flex justify-center items-end gap-x-2.5">
                                    @if(auth()->user()->isAdmin('edit-category'))
                                    ...
                                    @endif
                                    @if(auth()->user()->isAdmin('delete-category'))
                                        <a href="#"
                                           wire:click.prevent="openDeleteModal({{ $category->id }} , '{{ $category->name }}')"
            ...
            <div wire:loading.remove wire:target="openDeleteModal" class="w-full">
                ...
                <div class="flex flex-col">
                    ...
                    <div class="flex items-center gap-x-1.5">
                        ...
                        <!-- عنوان (h3) -->
                        <h3 class="text-lg leading-6 font-bold text-red-600 dark:text-red-400">تأیید حذف دسته بندی <span
                                class="text-indigo-600 dark:text-indigo-400">{{ $categoryNameToDelete}}</span></h3>
                    </div>

                    {{-- ردیف ۲: متن اصلی (با تورفتگی برای تراز) --}}
                    <div class="mt-3 mr-4">
                        <p class="text-sm text-gray-700 dark:text-gray-300">
                            آیا مطمئن هستید که می‌خواهید دسته بندی «<span
                                class="text-red-600 dark:text-red-400 font-medium">{{ $categoryNameToDelete }}</span>»
