<div class=""
     x-data="{
        ...
        showRoleModal: false,
        selectedUserName: '', // برای نمایش نام نقش
        selectedRoles: [] // برای نمایش لیست مجوزها
    }"
        ...
        <div class="card-body">
            ...
            <div class="flex flex-col-reverse md:flex-row md:justify-between items-start">
                ...
                <div class="flex gap-x-1 w-full md:w-max justify-between">
                    @if(auth()->user()->isAdmin('create-user'))
                        <a href="{{ route('panelAdmin.users.create') }}"
                           class="bg-green-500 text-white w-max px-3.5 py-2 rounded-lg">افزودن</a>
                    @endif
                    @if(auth()->user()->isAdmin('delete-user'))
                        <a href="{{ route('panelAdmin.users.soft.delete') }}"
                           class="bg-red-500 text-white w-max px-3.5 py-2 rounded-lg">کاربران حذف شده</a>
                    @endif
                </div>
            </div>
                    ...
                    <table class="box-table">
                        ...
                        @foreach($users as $user)
                            <tr class="table-item">
                                ...
                                <td>
                                 <span
                                     class="badge-custom-panel-form {{ $user->is_admin ? 'bg-green-100 text-green-800 border-green-400/30 px-1.5' : 'bg-blue-100 text-blue-800 border-blue-400/30 px-2.5' }}">
                                        {{ $user->roleLabel() }}
                                 </span>
                                </td>
                                <td>
                                ...
                                <td class="flex justify-center items-end gap-x-2.5">
                                    @if(auth()->user()->isAdmin('edit-user'))
            ...
            <div class="flex justify-center mt-10">
                <div class="flex space-x-2 rtl:space-x-reverse">
                    {{ $users->links('vendor.pagination.custom-pagination-panel-tailwind') }}
                </div>
            </div>
    ...
    <div x-show="showRoleModal" x-cloak
        ...
        <div x-show="showRoleModal"
                    ...
                    <ul class="grid grid-cols-1 sm:grid-cols-2 gap-3 text-center">
                        ...
                        <template x-for="role in selectedRoles" :key="role">
                            <li class="bg-indigo-50 dark:bg-gray-700 p-3 rounded-lg text-sm font-medium
                                       ...
                                       hover:bg-indigo-100 transition duration-150" x-text="role">
                            </li>
                        </template>
                    </ul>
                </template>
            ...
            <div class="mt-6 pt-4 border-t border-gray-200 dark:border-gray-700 flex justify-end">
                <button
                    x-on:click="selectedRoles = []; selectedUserName = ''; showRoleModal = false; document.body.style.overflow = 'auto';"
