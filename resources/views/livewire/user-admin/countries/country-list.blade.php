...
>
        ...
        <div class="card-body">
            ...
            <div class="flex flex-col-reverse md:flex-row md:justify-between items-start">
                ...
                <div class="flex gap-x-1 w-full md:w-max justify-between">
                    @if(auth()->user()->isAdmin('create-country'))
                        <a href="{{ route('panelAdmin.countries.create') }}"
                ...
                <div class="parent-table w-max md:w-full overflow-auto md:overflow-hidden" wire:key="user-list-table">
                    <table class="box-table">
                        ...
                        @foreach($countries as $country)
                            <tr class="table-item">
                                <td><span class="font-normal">{{ $country->name_en }}</span></td>
                                <td><span class="font-normal">{{ $country->name_fa }}</span></td>
                                <td><span class="font-normal">{{ $country->code }}</span></td>
                                <td class="flex justify-center items-end gap-x-2.5">
                                    @if(auth()->user()->isAdmin('edit-country'))
                                        <a href="{{ route('panelAdmin.countries.edite', $country->id ) }}"
                                    ...
                                    @endif
                                    @if(auth()->user()->isAdmin('delete-country'))
                                        <a href="#"
                                           wire:click.prevent="openDeleteModal({{ $country->id }} , '{{ $country->name_fa }}')"
            ...
            <div class="flex justify-center mt-10">
                <div class="flex space-x-2 rtl:space-x-reverse">
                    {{ $countries->links('vendor.pagination.custom-pagination-panel-tailwind') }}
                </div>
            </div>
    ...
    <div x-show="showDeleteModal" x-cloak
        ...
        <div x-show="showDeleteModal"
            ...
            <div wire:loading wire:target="openDeleteModal" class="flex justify-center items-center flex-col gap-y-1 h-full mx-auto">
            ...
            <div wire:loading.remove wire:target="openDeleteModal" class="w-full">
                ...
                <div class="flex flex-col">
                    ...
                    <div class="flex items-center gap-x-1.5">
                        ...
                        <!-- عنوان (h3) -->
                        <h3 class="text-lg leading-6 font-bold text-red-600 dark:text-red-400">تأیید حذف کشور <span
                                class="text-indigo-600 dark:text-indigo-400">{{ $countryNameToDelete }}</span></h3>
                    </div>

                    {{-- ردیف ۲: متن اصلی (با تورفتگی برای تراز) --}}
                    <div class="mt-3 mr-4">
                        <p class="text-sm text-gray-700 dark:text-gray-300">
                            آیا مطمئن هستید که می‌خواهید کشور «<span
                                class="text-red-600 dark:text-red-400 font-medium">{{ $countryNameToDelete}}</span>» را
