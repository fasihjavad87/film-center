<div
    ...
    x-on:show-episode-delete-modal.window="showEpisodeDeleteModal = true; document.body.style.overflow = 'hidden';"
    x-on:close-episode-delete-modal.window="showEpisodeDeleteModal = false; document.body.style.overflow = 'auto';"
>
    <div class="card mt-7">
        <div class="card-body">
            ...
            <div class="flex flex-col-reverse md:flex-row md:justify-between items-start">
                ...
                <div class="flex gap-x-1 w-full md:w-max justify-between">
                    @if(auth()->user()->isAdmin('create-episode'))
                        ...
                        @foreach($episodes as $episode)
                            <tr class="table-item">
                                ...
                                <td class="flex justify-center items-end gap-x-2.5">
                                    @if(auth()->user()->isAdmin('edit-episode'))
                                    ...
                                    @endif
                                    @if(auth()->user()->isAdmin('delete-episode'))
                                        <a href="#"
                                           wire:click.prevent="openDeleteModal({{ $episode->id }} , '{{ $episode->episode_number }}')"
                                           x-on:click="$dispatch('show-episode-delete-modal')"
    ...
    <div x-show="showEpisodeDeleteModal" x-cloak
        ...
        <div x-show="showEpisodeDeleteModal"
             x-transition:enter="transition ease-out duration-300"
            ...
            <div wire:loading.remove wire:target="openDeleteModal" class="w-full">
                ...
                <div class="flex flex-col">
                    ...
                    <div class="flex items-center gap-x-1.5">
                        ...
                        <!-- عنوان (h3) -->
                        <h3 class="text-lg leading-6 font-bold text-red-600 dark:text-red-400">تأیید حذف قسمت <span
                                class="text-indigo-600 dark:text-indigo-400">{{ $episodeNameToDelete }}</span></h3>
                    </div>

                    {{-- ردیف ۲: متن اصلی (با تورفتگی برای تراز) --}}
                    <div class="mt-3 mr-4">
                        <p class="text-sm text-gray-700 dark:text-gray-300">
                            آیا مطمئن هستید که می‌خواهید قسمت «<span
                                class="text-red-600 dark:text-red-400 font-medium">{{ $episodeNameToDelete }}</span>» را
                ...
                <div class="mt-6 pt-4 border-t border-gray-200 dark:border-gray-700 flex justify-end gap-3">
                    <button x-on:click="showEpisodeDeleteModal = false; document.body.style.overflow = 'auto';"
