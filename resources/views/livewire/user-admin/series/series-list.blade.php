        ...
        <div class="card-body">
            ...
            <div class="flex flex-col-reverse md:flex-row md:justify-between items-start">
                ...
                <div class="flex gap-x-1 w-full md:w-max justify-between">
                    @if(auth()->user()->isAdmin('create-series'))
                        <a href="{{ route('panelAdmin.series.create') }}"
                            ...
                            <tr class="table-item">
                                <td>
                                    <div class="flex justify-center">
                                        <img src="{{ asset('uploads/' . $ser->details->poster) }}" alt="image"
                                             class="table-avatar">
                                    </div>
                                </td>
                                <td><span class="font-normal">{{ $ser->title }}</span></td>
                                <td><span class="font-normal">{{ $ser->e_name }}</span></td>
                                <td><span class="font-normal">{{ $ser->slug }}</span></td>
                                <td>
                                    <span
                                        class="font-normal">{{ \Hekmatinasser\Verta\Verta::instance($ser->created_at)->format('Y/m/d') }}</span>
                                </td>
                                <td class="flex justify-center items-end gap-x-2.5">
                                    @if(auth()->user()->isAdmin('edit-series'))
                                        <a href="{{ route('panelAdmin.series.edite', $ser->id ) }}"
                                    ...
                                    @endif
                                    @if(auth()->user()->isAdmin('delete-series'))
                                        <a href="#"
                                           wire:click.prevent="openDeleteModal({{ $ser->id }} , '{{ $ser->title }}')"
            ...
            <div class="flex justify-center mt-10">
                <div class="flex space-x-2 rtl:space-x-reverse">
                    {{ $series->links('vendor.pagination.custom-pagination-panel-tailwind') }}
                </div>
            </div>
            ...
            <div wire:loading.remove wire:target="openDeleteModal" class="w-full">
                ...
                <div class="flex flex-col">
                    ...
                    <div class="flex items-center gap-x-1.5">
                        ...
                        <!-- عنوان (h3) -->
                        <h3 class="text-lg leading-6 font-bold text-red-600 dark:text-red-400">تأیید حذف سریال <span
                                class="text-indigo-600 dark:text-indigo-400">{{ $seriesNameToDelete }}</span></h3>
                    </div>

                    {{-- ردیف ۲: متن اصلی (با تورفتگی برای تراز) --}}
                    <div class="mt-3 mr-4">
                        <p class="text-sm text-gray-700 dark:text-gray-300">
                            آیا مطمئن هستید که می‌خواهید سریال «<span
                                class="text-red-600 dark:text-red-400 font-medium">{{ $seriesNameToDelete }}</span>» را
